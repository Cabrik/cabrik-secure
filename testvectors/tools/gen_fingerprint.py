...
ENC_LEN, SIG_LEN, MLKEM_LEN = 32, 32, 1184
...
def fingerprint(enc_pub: bytes,
    ...
    h.update(bytes([1 if mlkem_pub is not None else 0]))
    h.update(mlkem_pub if mlkem_pub is not None else bytes(MLKEM_LEN))
    return h.digest()
...
cases = [
    ("fp-full", "alle drei Schluessel vorhanden",
     b(0x01, ENC_LEN), b(0x02, SIG_LEN), b(0x03, MLKEM_LEN)),
    ("fp-no-sig", "Anonymitaets-Identitaet ohne Signierschluessel",
     b(0x01, ENC_LEN), None, b(0x03, MLKEM_LEN)),
    ("fp-no-mlkem", "aus v1 migrierter Kontakt ohne PQ-Schluessel",
    ...
    ("fp-zero-mlkem", "ML-KEM-Schluessel aus lauter Nullen",
     b(0x01, ENC_LEN), None, bytes(MLKEM_LEN)),
...
fp_vectors = []
computed = {}
for vid, desc, enc, sig, mlkem in cases:
    fp = fingerprint(enc, sig, mlkem)
    computed[vid] = fp
    ...
    fp_vectors.append({
        ...
        "input": {
            ...
            "has_mlkem": mlkem is not None,
            "mlkem_pub_byte": (mlkem[0] if mlkem else None),
        },
        "expected": {
...
doc = {
    ...
    "notes": [
        ...
        "eines einzelnen Bytes; enc_pub_byte, sig_pub_byte und mlkem_pub_byte",
        "geben dieses Byte an. has_sig bzw. has_mlkem = false bedeutet, dass",
...
assert computed["fp-neither"] != computed["fp-zero-mlkem"], "Praesenz-Byte mlkem wirkungslos"
